...
imprimeEncabezado();
?>
<p class="yacomas_login">Login: <?php print $_SESSION['YACOMASVARS']['asilogin']?>
...
$userQuery = "SELECT nombrep FROM asistente WHERE id='$idasistente'";
...
$msg= stripslashes($asistente_name).", solo puedes tener 
        m&aacute;ximo $max_inscripcionTA inscripciones a talleres  y <br>
        m&aacute;ximo $max_inscripcionTU inscripciones a tutoriales .";
?>
...
// Primero checamos si los valores elegidos solo son los requeridos 
            ...
            for ($comparado=0,$i=0; $i < $r; $i++,$comparado++) {
                for ($x=0; $x < $comparado; $x++) {
                    if ($llave[$i]==$horaocup[$x]) {
                        $conflicto=1;
                        $errmsg2.='<br>
                        <p class="yacomas_error">
                        Los conflictos son entre: '.$nomt[$x].' y '.$nomt[$i].'</p>'; 
                    }
                }
                if ($conflicto!=1)
                    ...
                    $errmsg='
                        <li>Existen conflictos de cruce de horarios entre los talleres a los que 
                        intentas inscribirte y/o los talleres que tienes inscritos'.$errmsg2;
                        break;
                }
            }
	}
	...
	if (!empty($errmsg)) {
            ...
            for ($i=0; $i < $eventos;$i++) {
                    $queryI = 'INSERT INTO inscribe(id_asistente,id_evento,reg_time) VALUES ('.
                                $idasistente.','.$evento[$i]['id_evento'].','.$date.')';
                    $resultI = mysql_query($queryI) 
                                or err("No se puede insertar taller".mysql_errno($resultI));
                    print '<tr><td class="resultado">'.$evento[$i]['nombre'].'</tr>';
                    retorno();
            }
            print '</table><hr>';	
	}
...
$fechaRecords = mysql_query($fechaQueryE) or err("No se pudo listar fechas 
    de eventos ".mysql_errno($fechaRecords));
...
$inscritosTARecord= mysql_query($QTAinscritos) or err("No se pudo listar talleres inscritos 
    evento".mysql_errno($inscritosTARecord));
//print $QTAinscritos;
...
$QTUinscritos='	SELECT 	count(*) 
		...
		WHERE	I.id_evento=E.id AND
			...
			I.id_asistente="'.$idasistente.'"';
$inscritosTURecord= mysql_query($QTUinscritos) or err("No se pudo listar talleres 
    inscritos evento".mysql_errno($inscritosTURecord));
//print $QTUinscritos;
...
$msg0='<h3>Observaciones sobre los talleres:</h3> 
    <ul>
        <li>Para los talleres es necesario llevar equipo propio (desktop, laptop, netbook).</li>
        <li>Trata de llegar 30 minutos antes del inicio de taller para instalar y configurar
            el software a usar, para que así no tengas problemas durante el mismo.</li>
        <li>Recuerda cumplir los prerequisitos para ingresar a talleres y tutoriales </li>
        <li>Para ver informaci&oacute;n sobre algun tema o ponente haz clik en el el nombre 
            de taller o tutorial que sea de tu inter&eacute;s</li> </ul>';
$msg1='Actualmente estas inscrito en '.$TAinscritos.' talleres y '.$TUinscritos.' tutoriales';
retorno();
$msg1.='<br>Puedes inscribirte a '.$inscrip_TA_libres.' talleres y '.$inscrip_TU_libres.' tutoriales
    mas';
$msg2='<h3> Tips para inscribirte: </h3>
        <ul>
        <li>El n&uacute;mero a la derecha del taller indica los lugares diponibles.</li>
        <li>Si este n&uacute;mero es 0 indica cupo lleno, si este n&uacute;mero es negativo 
            indica "sobrecupo" y no apacer&aacute; la casilla para elegir el taller.</li>
        </ul>';
